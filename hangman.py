...
#      |
#  ____|___
# Creating function to display the hangman art based on lives/chances left
print(''' 
 _                                             
| |                                            
...
print("The rules are as follows: You have to find the word by guessing a letter in each turn.")
print("If you guess right, we'll show you the position your guessed letter is in.")
print("If you guess wrong, you'll lose lives which will be depicted by hangman slowly being completed part by part.")
print("You win if you manage to guess the word. And you lose if you let the little man die")
print("Enjoy!!")
def hang_man_art():
     #if lives == 7:
     #   print('''
...
#Making the game in an infinite loop so that the player can choose to play it without rerunning the code
while True:
    ...
    #print(word)
    # To find the required number of blanks
    blanks=[]
    for letter in word:
        blanks.append('_')
    # Prints the blanks before asking the user to choose
    blank = ' '.join(blanks)
    print(blank)
    ...
    while (blanks.count('_')>0) and (lives>0) :
        ...
        # Parting the String into Characters
        check_word = []
        # To keep track of the position of the guessed letter
        guess_pos = []
        i = 0
        for letter in word:
            ...
            if letter == guess:
                guess_pos.append(i)
            i+=1
        # Else if the guessed letter is wrong then we must take away the life
        ...
        # Replace the blanks with the guessed letter if the guess is correct

        while(occurences > 0):
            hang_man_art()
            blanks[guess_pos[occurences-1]] = guess
            occurences -= 1
        blank = ' '.join(blanks)
        print(blank)
    if (lives == 0):
